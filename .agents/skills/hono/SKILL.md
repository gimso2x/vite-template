---
name: hono
description: Use when building Hono web applications or when the user asks about Hono APIs, routing, middleware, JSX, validation, testing, or streaming. TRIGGER when code imports from 'hono' or 'hono/*', or user mentions Hono. Use `pnpm dlx hono request` to test endpoints.
---

## 🚨 Project Specific Conventions (Ssuk / Vite Template)

- **Package Manager**: MUST use `pnpm` (e.g., `pnpm install`, `pnpm add`). Do NOT use npm/yarn.
- **Imports**: Prefer absolute paths with `@/` alias.
- **Naming**:
  - Files: `kebab-case.ts/tsx`
  - Components/Types: `PascalCase`
  - Hooks/Functions: `camelCase`
- **Routing**: TanStack Router is configured in `src/routes/`. Never manually edit `src/routeTree.gen.ts`.
- **Validation**: Always run `pnpm lint`, `pnpm format`, and `pnpm build` before committing.
- **Reference**: Follow `AGENTS.md` and `docs/conventions/coding-convention.md`.

# Hono Skill

Build Hono web applications. This skill provides inline API knowledge for AI. Use `pnpm dlx hono request` to test endpoints. If the `hono-docs` MCP server is configured, prefer its tools for the latest documentation over the inline reference.
...
```bash
# GET request
pnpm dlx hono request [file] -P /path

# POST request with JSON body
pnpm dlx hono request [file] -X POST -P /api/users -d '{"name": "test"}'
```
...
```bash
pnpm dlx workers-fetch /path
pnpm dlx workers-fetch -X POST -H "Content-Type:application/json" -d '{"name":"test"}' /api/users
```
...
Use `jsxRenderer` middleware for layouts. See `pnpm dlx hono docs /docs/middleware/builtin/jsx-renderer` for details.
...
For details, use `pnpm dlx hono docs /docs/helpers/<helper-name>`.
